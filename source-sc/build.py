...
IVERILOG_ROOT = Path(r"D:\iverilog")
IVERILOG_BIN = IVERILOG_ROOT / "bin"
TOP = "sccomp_tb"
...
def require_tool(name: str) -> str:
    ...
    if path is None:
        candidate = IVERILOG_BIN / f"{name}.exe"
        if candidate.exists():
            return str(candidate)
    if path is None:
...
def run_command(cmd: list[str]) -> None:
    ...
    if IVERILOG_BIN.exists():
        env["PATH"] = f"{IVERILOG_BIN};" + env.get("PATH", "")
